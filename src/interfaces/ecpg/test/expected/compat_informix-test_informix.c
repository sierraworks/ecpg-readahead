...
#line 46 "test_informix.pgc"
 ...
 ECPG_informix_reset_sqlca(); /* declare c no scroll cursor for select * from test where i <= $1  */
#line 49 "test_informix.pgc"
	...
	while (1)
	{
		{ ECPGfetch(__LINE__, 1, 1, NULL, 0, ECPGc_forward, "1", 0, "c", ECPGst_normal, "fetch forward c", ECPGt_EOIT, 
	ECPGt_int,&(i),(long)1,(long)1,sizeof(int), 
...
static void openit(void)
{
	{ ECPGopen(__LINE__, 1, 1, NULL, 0, 0, ECPGcs_no_scroll, 1, 1, 0, "c", ECPGst_normal, "declare c no scroll cursor for select * from test where i <= $1 ", 
